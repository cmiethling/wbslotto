...
public class ZiehungAuswerten implements ZiehungAuswertenLocal {
         ...
         * 
         * Aenderungen im Code:
         * -in KostenErmittelnLocal einfuegen:
         * Gebuehr findGebuehrForSpielTag(List<Gebuehr> gebuehren, Date spielTag);
         * + diese Meth in KostenErmitteln public machen (schon als private
         * vorhanden)!
         * 
         ...
         * // populateLottoscheinZiehung6Aus49.populateLottoscheinZiehung6Aus49();
         * 
         * 
         * TODO
         * -bei hoher Datenmenge (>=100_000 lottoscheine) schafft es GarbargeCollector
         * nicht mehr
         * >> wegen zB ziehung.getLottoscheinziehungList() >> bei 100_000
         * lottoscheinziehungen kann RAM (oder Heap?) overloaded werden
         * (java.lang.OutOfMemoryError: GC overhead limit exceeded)
         * Lsg: Haeppchenweise Daten uebermitteln mittels facade.findRange()!!!
         * 
         */
...
//                ############## Log #############
                log.log(Level.INFO, "");
                log.log(Level.INFO, "AnzahlGewinner " + SUPER6 + ": "
                        + getMapKeyAsGewinnklassenr(SUPER6, anzGewinnerProKlasse));
                log.log(Level.INFO, "AnzahlGewinner " + SPIEL77 + ": "
...
//                gewinnklassen = jpaQueries.findGewinnklassenForZiehung(zie);
                ...
                log.log(Level.INFO,
                        "ZiehungsId: " + zie.getZiehungid() + "   " + SPIEL77 + ": "
...
// gkl1 mit Prozente: zB 20% von der Haelfte des Einsatzes (ohne Jackpot)
                                lz.setGewinnspiel77(BigInteger
                                        .valueOf(setBetragForGkl1ForSpiel77(SPIEL77_GKL1_ANTEIL_IN_PROZENT)));
                        }
        ...
        private void createLottoscheinziehung6aus49(Lottoscheinziehung lz) {
                ...
                for (int i = 0; i < tippsAsLongArray.length; i++) {
                        ...
                        if (gkl6Aus49 > 0) {
                                ...
                                Optional<Gewinnklasse> gklLottoOpt = findGklForScheinZie(SECHS_AUS_49,
...
//                                                + ")");
        ...
        private void updateZiehung() {
                ...
                log.log(Level.INFO, "AnzSuper6 * gebuehr = Einsatz: " + anzSuper6 + " * "
                        + geb.getEinsatzsuper6() + " = " + einsatzSuper6);
                log.log(Level.INFO, "AnzSpiel77 * gebuehr = Einsatz: " + anzSpiel77 + " * "
                        + geb.getEinsatzspiel77() + " = " + einsatzSpiel77);
                log.log(Level.INFO,
                        "GesamtAnzTipps * gebuehr = Einsatz: " + summeAnzahlTipps + " * "
...
//        Gkl1 berechnen + abziehen
                uebrigeVerteilung = setBetragForGkl(gklsLotto[0], prozenteMal100[0],
                        uebrigeVerteilung, anzGewinnerProKlasse);
...
//                ########### ZwischenLog ##############
                log.log(Level.INFO,
                        "EinsatzLotto/" + ABZUG_EINSATZ_LOTTO + ": "
                                + (einsatzLotto / ABZUG_EINSATZ_LOTTO) + "    gkl1abgezogen: "
                                + gkl1abgezogen +

                                "     anzGkl9 * gebuehr = gkl9abgezogen: " + anzGkl9 + " * "
                                + gkl9.getBetrag()
                                        .intValue()
                                + " = " + (anzGkl9 * gkl9.getBetrag()
                                        .intValue()));
                log.log(Level.INFO,
                        "uebrigeVerteilungFuerGkl2-8: " + (einsatzLotto / ABZUG_EINSATZ_LOTTO)
                                + " - " + gkl1abgezogen + " - " + (anzGkl9 * gkl9.getBetrag()
                                        .intValue())
                                + " = " + uebrigeVerteilung);
//                ###################################
...
//                Gkl2-8 berechnen
                for (int i = 1; i < gklsLotto.length - 1; i++) {
                        setBetragForGkl(gklsLotto[i], prozenteMal100[i], uebrigeVerteilung,
...
//                                .getIsabsolut())
                lzLottosOhneGkl9.forEach(lzLotto -> {
                        Gewinnklasseziehungquote gklZie = gklZieQuos.stream()
                                .filter(g -> g.getGewinnklasseid()
                                        .getGewinnklasseid() == lzLotto.getGewinnklasseid()
                                ...
                                .orElseThrow(() -> new NoSuchElementException(
                                        "invalid record in gewinnklasseziehungsquote or "
                                                ...
                                                + lzLotto.getGewinnklasseid()
                                                        .getGewinnklasseid()
                                                + "   gklZieQuosGklIds: " + gklZieQuos.stream()
                                                        .map(g -> g.getGewinnklasseid()
                                                                .getGewinnklasseid())
...
//############## Helper Methods ################################
        ...
        private long setBetragForGkl(Gewinnklasse gkl, int prozenteMal100,
                long uebrigeVerteilung, Map<Gewinnklasse, Integer> anzGewinnerProKlasse) {
...
//                        TODO: createJackpot(gklAnteil, gkl, (zie))
                ...
                log.log(Level.INFO,
                        "Gkl: " + gkl.getGewinnklassenr()
                                + " >> gklAnteil / anzGew(bei 0>>Jackpot) = quoteGlk:  " + gklAnteil
                                + " / " + anzGew + " = " + quoteGkl + "   >>  Gkl "
                                + gkl.getGewinnklassenr() + ": " + (prozenteMal100 / 100.0) + "% von "
