...
//                DateTimeFormatter formatter = DateTimeFormatter.ofLocalizedDate(FormatStyle.SHORT);
	...
	private static Table createTableRechnung(Auftrag auftrag) {
		...
		table.addCell(createTextCell("1", 30, TextAlignment.JUSTIFIED)
				...
				.setBorderRight(new SolidBorder(color, .1f)));
		table.addCell(
				createTextCell(myFormatter.format(auftrag.getKosten() / 100.0) + " €",
						30, TextAlignment.JUSTIFIED)
								.setBorderBottom(new SolidBorder(color, .1f)));
		return table;
	}
	...
	private static Table createTableZahlungseinzelheiten(
		...
		table
				.addCell(createTextCell("Gültig ab Datum", 100, TextAlignment.JUSTIFIED)
						...
						.setBorderRight(new SolidBorder(color, .1f)));
		table.addCell(
				createTextCell("Anzahl Ziehungen", 100, TextAlignment.JUSTIFIED)
						.setBorderBottom(new SolidBorder(color, .1f))
						.setBorderRight(new SolidBorder(color, .1f)));
		table.addCell(
				createTextCell("Einsatz pro Tipp", 100, TextAlignment.JUSTIFIED)
						.setBorderBottom(new SolidBorder(color, .1f))
						.setBorderRight(new SolidBorder(color, .1f)));
		table.addCell(
				createTextCell("Einsatz Spiel 77", 100, TextAlignment.JUSTIFIED)
						.setBorderBottom(new SolidBorder(color, .1f))
						.setBorderRight(new SolidBorder(color, .1f)));
		table
...
//                        table.addCell(
//                                        createTextCell(date.format(DateTimeFormatter.ofPattern(dateTimeFormat)), 100, TextAlignment.JUSTIFIED)
		...
		gebueren.forEach((date, gebuehr) -> {
			...
			table.addCell(createTextCell(String.valueOf(gebuehr.getEinsatzprotipp()),
					100, TextAlignment.JUSTIFIED)
							.setBorderBottom(new SolidBorder(color, .1f))
							.setBorderRight(new SolidBorder(color, .1f)));
			table.addCell(createTextCell(String.valueOf(gebuehr.getEinsatzspiel77()),
					100, TextAlignment.JUSTIFIED)
							.setBorderBottom(new SolidBorder(color, .1f))
							.setBorderRight(new SolidBorder(color, .1f)));
			table.addCell(createTextCell(String.valueOf(gebuehr.getEinsatzsuper6()),
					100, TextAlignment.JUSTIFIED)
							.setBorderBottom(new SolidBorder(color, .1f))
							.setBorderRight(new SolidBorder(color, .1f)));
		});
