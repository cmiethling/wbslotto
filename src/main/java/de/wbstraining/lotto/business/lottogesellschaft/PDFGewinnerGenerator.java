...
public class PDFGewinnerGenerator {
	...
	public static final String gratulation = "../standalone/deployments/wbslotto.war/resources/images/gratulation.jpg";
		// Table : getBelegnummer , getLosnummer , Betrag : getGewinnspiel77 oder getGewinnsuper6
	
	public static synchronized void createPDFAsByteArray(AuftragGewinner auftrag) {//byte[] 
...
//		Document document = new Document(pdf);
		...
		try {
			writer = new PdfWriter(new FileOutputStream(DEST +auftrag.getName() +".pdf"));
			PdfDocument pdf = new PdfDocument(writer);
			Document document = new Document(pdf);
		
			// add image
			try {
				document.add(createImageCell(gratulation)).setWidth(1024);
			} catch (MalformedURLException e) {
				e.getMessage();
			}
			//add betreff
			document.add(new Paragraph("Gewinne im Lotto Spiel!").addStyle(new Style().setFontSize(30)).setTextAlignment(TextAlignment.CENTER).setBold());
		// add message
				document.add(new Paragraph("Sehr geehrte Frau / geehrter Herr : "
		+ auftrag.getName() + " " + auftrag.getVorname() +",\r\n"
						+ "Sie haben " +String.valueOf(betrag)+" € an der Ziehung von " + auftrag.getZiehungDatum() +
						" mit Belegnummer: "+auftrag.getBelegnummer() +" und Losnummer: "+auftrag.getLosnummer()+" gewonnen im Spiel "
		+ spiel +"."));
